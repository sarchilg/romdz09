...
public class Encoder
{
	...
	public void WriteCoderProperties(java.io.OutputStream outStream) throws IOException
	{
		properties[0] = (byte)((_posStateBits * 5 + _numLiteralPosStateBits) * 9 + _numLiteralContextBits);
                int ii=(_posStateBits * 5 + _numLiteralPosStateBits) * 9 + _numLiteralContextBits;
                myLog.Systemoutprintln(""+_posStateBits);
                myLog.Systemoutprintln(""+_numLiteralPosStateBits);
                myLog.Systemoutprintln(""+_numLiteralContextBits);
                myLog.Systemoutprintln(""+ii);
                myLog.Systemoutprintln(""+String.format("%02X ",ii));
                myLog.Systemoutprintln("_dictionarySize="+_dictionarySize);
		for (int i = 0; i < 4; i++){
                    int dd=_dictionarySize >> (8 * i);
			properties[1 + i] = (byte)(dd);
                        myLog.Systemoutprintln(""+i+"=="+dd+"="+String.format("%02X ",properties[1 + i]));
                }
