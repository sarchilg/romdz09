...
public class Decoder
{
	...
	public SevenZip.Compression.RangeCoder.Decoder m_RangeDecoder = new SevenZip.Compression.RangeCoder.Decoder();
	...
	boolean SetDictionarySize(int dictionarySize)
	{
		...
		if (m_DictionarySize != dictionarySize)
		{
			...
			da=m_OutWindow.Create(Math.max(m_DictionarySizeCheck, (1 << 12)));
		}
		return da;
	}
			...
			long outSize) throws IOException
	{
			...
			if (m_RangeDecoder.DecodeBit(m_IsMatchDecoders, (state << Base.kNumPosStatesBitsMax) + posState) == 0)
			{
				...
				if (!Base.StateIsCharState(state))
					...
					prevByte = decoder2.DecodeNormal(m_RangeDecoder);
                                if (!m_RangeDecoder.da) {
                                    //myLog.Systemoutprintln("mybreak");
                                    break;}
			...
			else
			{
				...
				else
				{
					...
					int posSlot = m_PosSlotDecoder[Base.GetLenToPosState(len)].Decode(m_RangeDecoder);
                                        //System.out.println("posSlot="+posSlot+"==="+Base.kStartPosModelIndex+"==="+Base.kEndPosModelIndex);
					if (posSlot >= Base.kStartPosModelIndex)
					{
						int numDirectBits = (posSlot >> 1) - 1;
						rep0 = ((2 | (posSlot & 1)) << numDirectBits);
                                                long rr=((2 | (posSlot & 1)) << numDirectBits);
                                                //System.out.println("111==="+rep0+"="+numDirectBits+"=="+(2 | (posSlot & 1))+"=="+rr);
						...
						else
						{
							...
							if (rep0 < 0)
							{
                                                            //System.out.println("rep0<0==="+rep0);
								if (rep0 == -1)
									break;
                m_OutWindow.Flush();//?????????????
		...
		m_RangeDecoder.ReleaseStream();
				...
				if (rep0 >= nowPos64 || rep0 >= m_DictionarySizeCheck)
				{
                                    ...
                                    //System.out.println("rep0=!=="+rep0+"="+nowPos64+"="+m_DictionarySizeCheck+"="+len+"="+m_RangeDecoder.bb);
					// m_OutWindow.Flush();5d 00 00 01 00
                                
                                    if (nowPos64 >= m_DictionarySizeCheck)
		...
		m_RangeDecoder.ReleaseStream();    
					return false;
                                    ...
                                    else{System.out.println("33="+rep0+"="+(int)nowPos64);
               // m_OutWindow.Flush();//?????????????
		m_OutWindow.ReleaseStream();
		m_RangeDecoder.ReleaseStream();    
                                        return false;
            //                            rep0 = (int)nowPos64;//-len;
                                    }
				}//else System.out.println("rep0==="+rep0+"="+nowPos64+"="+m_DictionarySizeCheck+"="+len+"="+m_RangeDecoder.bb);
	...
	{
            for (int i=0;i<properties.length;i++) myLog.Systemoutprintln(""+String.format("%02x ",properties[i] & 0xFF));
		if (properties.length < 5)
			return false;
                //myLog.Systemoutprintln("1111111");
		...
		for (int i = 0; i < 4; i++)
			dictionarySize += ((int)(properties[1 + i]) & 0xFF) << (i * 8);
                //myLog.Systemoutprintln("22222222222222");
		if (!SetLcLpPb(lc, lp, pb))
			return false;
                //myLog.Systemoutprintln("3333333333333==="+dictionarySize);
                //dictionarySize=dictionarySize*2;
                //myLog.Systemoutprintln("3333333333333==="+dictionarySize);
                //System.out.println("dictionarySize="+dictionarySize);
                //if (dictionarySize>50000000) return false;
